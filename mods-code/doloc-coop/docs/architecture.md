...
箱子、作物、任务、掉落物各写了一遍**一模一样的骨架**:
计时 → 扫描本地 → 算指纹 → 和上次比 → 只发变化的;
对端按 id 匹配 → 指纹一致就跳过 → 否则应用。

抄四遍的代价不只是重复代码:
- "指纹初值用空串导致永不广播"这个 bug,**DropItemSync 和 MissionSync 都中了同一枪**,
  得在每个类里各修一次。
- 每加一个同步域要在 `Plugin.cs` 动四处(Tick、ResendAll、事件订阅、面板显示),
  漏一处就是"功能写了但不生效",而且症状是静默的。

### 方案

`CoopCore.Replication` 下两个类:

- **`ReplicatedDomain<T>`** —— 骨架只有这一份。子类只描述三件事:
  怎么读(`ReadAll`/`IdOf`/`SignatureOf`)、怎么编解码(`Encode`/`Decode`)、怎么写回(`ApplyOne`)。
- **`SyncRegistry`** —— 统一入口。`Register(new XxxDomain())` 一行搞定,
  计时、差分、批量、重发、按频道路由全自动。

消息层也收敛了:各领域不再各占一个 `MsgType`,统一走
`[DomainSync][channel:1][payload]`。注册时校验频道号重复并**直接抛异常** ——
枚举值撞车曾让"发 A 实际发出 B",这类错误必须在注册那一刻炸掉,
而不是运行时表现成灵异现象。

### 效果

`CropSync`(约 190 行)→ `CropDomain`(约 130 行),少掉的正是那套骨架;
`Plugin.cs` 里的四处接线缩成注册表里的一行。
以后骨架改进(限流、冲突处理、增量压缩)所有域一起受益。

实测:`[Sync] 已注册同步域 作物(频道 1)` → `[作物] 已广播 1 条变化`,
新人进房 → `[Sync] 已要求 1 个同步域重发全量状态` → 重发正常。

### 迁移状态

- ✅ 作物 → `CropDomain`
- ⬜ 箱子 / 任务 / 掉落物 —— 结构相同,按同一模式迁即可
  (掉落物多一条客机→主机的捡拾上报,框架需要再加一个反向通道)
